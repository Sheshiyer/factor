---
name: fabric
description: "Fabric. Reusable prompt patterns."
version: 0.1.0
author: Thoughtseed
license: MIT
metadata:
...
Load this only when `connectors/enabled.yaml` lists `fabric`.
...
1. Load `connector-gate` and confirm `fabric` is enabled.
2. If the upstream is not installed, give the founder this command and stop:

   `npx skills add https://github.com/danielmiessler/Fabric`
