---
name: postiz-agent
description: "postiz-agent. Social publishing. A post waits for approval."
version: 0.1.0
author: Thoughtseed
license: MIT
metadata:
...
Social publishing. A post waits for approval.

Load this only when `connectors/enabled.yaml` lists `postiz-agent`.
...
1. Load `connector-gate` and confirm `postiz-agent` is enabled.
2. If the upstream is not installed, give the founder this command and stop:

   `npx skills add https://github.com/gitroomhq/postiz-agent`
