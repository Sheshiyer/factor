...
description: "platform-design. Platform design skills from the 20-skill stack."
version: 0.1.0
author: Thoughtseed
license: MIT
metadata:
...
Platform design skills from the 20-skill stack.

Load this only when `connectors/enabled.yaml` lists `platform-design`.
...
1. Load `connector-gate` and confirm `platform-design` is enabled.
2. If the upstream is not installed, give the founder this command and stop:

   `npx skills add https://github.com/ehmo/platform-design-skills`
