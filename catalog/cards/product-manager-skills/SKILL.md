...
description: "product-manager-skills. Product manager skills from the 20-skill stack."
version: 0.1.0
author: Thoughtseed
license: MIT
metadata:
...
Product manager skills from the 20-skill stack.

Load this only when `connectors/enabled.yaml` lists `product-manager-skills`.
...
1. Load `connector-gate` and confirm `product-manager-skills` is enabled.
2. If the upstream is not installed, give the founder this command and stop:

   `npx skills add https://github.com/Digidai/product-manager-skills`
