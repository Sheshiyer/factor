---
name: skill-seekers
description: "Skill Seekers. Builds a skill from documentation. Review the generated skill before enabling it."
version: 0.1.0
author: Thoughtseed
license: MIT
metadata:
...
Builds a skill from documentation. Review the generated skill before enabling it.

Load this only when `connectors/enabled.yaml` lists `skill-seekers`.
...
1. Load `connector-gate` and confirm `skill-seekers` is enabled.
2. If the upstream is not installed, give the founder this command and stop:

   `npx skills add https://github.com/yusufkaraaslan/Skill_Seekers`
