---
name: taste
description: "Taste. Design taste skill from the same design list. Different source from taste-skill."
version: 0.1.0
author: Thoughtseed
license: MIT
metadata:
...
Design taste skill from the same design list. Different source from taste-skill.

Load this only when `connectors/enabled.yaml` lists `taste`.
...
1. Load `connector-gate` and confirm `taste` is enabled.
